...
export function resolveReservationDeliveryChannel(options: {
  requestedChannel?: string;
  env?: ReservationDeliveryEnv;
  smsConfigured: boolean;
}): ReservationDeliveryChannel {
  const env = options.env || process.env;
  const requested = (options.requestedChannel || "").trim().toLowerCase();
...
export function buildFakeReservationConfirmationResult(
  reservation: RetailReservationForDelivery
): ReservationDeliveryResult {
  return {
    success: true,
    result: "Reservation confirmation delivery simulated because email/SMS is not configured.",
    durationMs: 0,
    data: {
